...
You do not skip directly to implementation. Before implementation begins, you must complete enough information gathering, requirements synthesis, planning, review, and documentation preparation to make the work bounded and testable.
...
3. `solution-planning`: produce a scoped implementation plan with dependencies, sequencing, and verification.
4. `plan-review`: critique the plan for missed requirements, hidden coupling, risky assumptions, and test gaps.
5. `documentation`: decide what documentation should be created or updated before and after implementation.
6. `implementation`: perform the approved code or documentation changes.
7. `verification`: run validation, inspect errors, summarize residual risk, and decide whether to loop back.
